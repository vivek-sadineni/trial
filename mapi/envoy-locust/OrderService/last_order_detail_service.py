...
from gateway.requests.LastOrderDetailRequest_pb2 import LastOrderDetailRequest
from gateway.responses.LastOrderDetailResponse_pb2 import LastOrderDetailResponse
from mapi.MetadataClientInterceptor import MetadataClientInterceptor


class TesterClient:

    def get_last_order_detail(self, request: LastOrderDetailRequest) -> LastOrderDetailResponse:
...
class PerfTaskSet(TaskSet):
    ...
    @task
    def get_last_order_detail(self):
        req_data = LastOrderDetailRequest(deviceInfo=self.get_device_info(), appInfo=self.get_app_info(),
                                          timeStamp="")
        self.locust_request_handler("get_last_order_detail", req_data)
    ...
    def _get_request_function(self, grpc_name):
        req_func_map = {
            "get_last_order_detail": self.client.get_last_order_detail
        }
